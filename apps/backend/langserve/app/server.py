...
from common.prompts import AGENT_DOCSEARCH_PROMPT, CUSTOM_CHATBOT_PROMPT, WELCOME_MESSAGE
...
# llm = AzureChatOpenAI(deployment_name=os.environ.get("AZURE_OPENAI_MODEL_NAME"), temperature=0, max_tokens=COMPLETION_TOKENS, streaming=True)

# # Initialize our Tools/Experts
# hrdocs_indexes = ["cogsrch-index-hrdocs"]
# hrdocs_search = GetDocSearchResults_Tool(name="hrdocs", indexes=hrdocs_indexes, k=5, description="It is useful for searching company information about health benefit, policies of company, roles description", reranker_th=1, sas_token=os.environ['BLOB_SAS_TOKEN'])

# techdocs_indexes = ["cogsrch-index-techdocs"]
# techdocs_search = GetDocSearchResults_Tool(name="techdocs", indexes=techdocs_indexes, k=5, description="It is useful for searching technical information about Azure Service like Azure AI Search, Azure AI Services", reranker_th=1, sas_token=os.environ['BLOB_SAS_TOKEN'])
...
# add_routes(
#     app,
#     agent_with_chat_history.with_types(input_type=Input, output_type=Output),
#     path="/agent",
# )
